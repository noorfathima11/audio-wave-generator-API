...
document.querySelector('#upload-button').addEventListener('click', function () {
  ...
  document.querySelector('#get-data-container').style.display = 'block'
  document.querySelector('#get-data-only').addEventListener('click', function(){
    request.open('GET', 'http://localhost:3000/soundWave', true)
    request.send(null)
  ...
  /*request.upload.addEventListener('progress', function (e) {
    console.log('eloaded', e.loaded)
    console.log('ecomplete', e.complete)
    let percentComplete = Math.floor(e.loaded / e.complete) * 100 + '%'
